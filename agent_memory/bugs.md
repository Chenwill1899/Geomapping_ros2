...
- `nf_yaw_light_filter` 当前只在 seeds `424242-424246` 上验证，5/5 到达且指标优于基线，但还未扩展到更大 seed sweep。
- `nf_yaw_light_filter` 的 seed `424243` arrival 从基线 `46.8s` 增至 `56.7s`，虽然 5-seed 均值只慢约 `2.7%`，但单 seed 有退化。
...
- H-FDM 当前 profile 缺少控制平滑/横向/角速度/jerk 正则和 command filter，离线指标显示 H-FDM 两组控制摆动明显高于 nominal。
...
- 是否将 `nf_yaw_light_filter` 提升为正式 source config，或先扩大 seed sweep。
- 是否扩大 `nf_green_mild_noise_jerk` 到完整 seeds `424242-424246` 复验，确认它可替代 v4 tight-noise 候选。
